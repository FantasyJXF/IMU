...
{
	while(USART_GetFlagStatus(macUSARTx,USART_FLAG_TXE)==RESET); //循环发送,直到发送完毕
	USART_SendData(macUSARTx,c);
}
...
static void handle_input(void)
{
    ...
    char c = USART_ReceiveData(macUSARTx);
    MPU_DEBUG_FUNC();
    switch (c) {
...
int main(void)
{
  ...
  ILI9341_DispString_EN_CH ( 20, 20, (const uint8_t *)"This is a MPU6050 demo", macBACKGROUND, macRED );
	...
	result = mpu_init(&int_param);
	  if (result) {
		  MPL_LOGE("Could not initialize gyro.result =  %d\n",result);
			ILI9341_DispString_EN_CH ( 0, 40, (const uint8_t *)"No MPU6050 detceted!Please check the hardware connection.", macBACKGROUND, macBLUE );

	  }
		else
		{
			ILI9341_DispString_EN_CH(30,40,(const uint8_t *)"MPU6050 decteted!", macBACKGROUND,macRED);
		}
  ...
  while(1){
    ...
    int new_data = 0;
    if (USART_GetFlagStatus (macUSARTx, USART_FLAG_RXNE)) {
        /* A byte has been received via USART. See handle_input for a list of
         * valid commands.
         */
		
        USART_ClearFlag(macUSARTx, USART_FLAG_RXNE);
